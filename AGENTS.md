...
- `backend/` — Go HTTP service (deployed on Cloud Run, GCP project `canzoniere-506115`) that proxies the public site to the GitHub API without exposing tokens: `POST /api/songs` turns an edited song into a branch + commit + pull request, `POST /api/suggestions` opens an issue. No database; abuse control via in-memory rate limiting and optional Cloudflare Turnstile. Plain `net/http`, no external deps. Run: `cd backend && GITHUB_TOKEN=... go run .`; tests: `go test ./...`. Deploy instructions in `backend/README.md`.
...
- `s/[category]/[slug]/` — the song sheet (transpose, chord display, per-song prefs). With student mode on it also shows the readiness line, the tappable chords (`SongSheet`'s `onchord`) and the "Studia" panel.
...
- `impostazioni/` — device settings; today the only one is student mode (`student.ts`).
- `crediti/` — credits and rights notice.

Modules (`src/lib/`): `data.ts` (songs and songbooks bundled via `import.meta.glob`, plus `allSongs`/`categories`/`findSong`), `search.ts`, `prefs.ts`, `favorites.ts`, `theme.ts`, `feedback.ts` (`mailto:` builder for `canzoniere@alessandriascout.it`), `collection.ts` (set lists encoded into the URL) and `saved-collections.ts` (the sets kept on the device, both the ones built in-app and the ones received by link/QR, listed in the menu under "Scalette temporanee"), `known-chords.ts` (the chords the reader can play, plus `readiness()` and `chordChanges()` for one song), `student.ts` (the student-mode flag), `study.ts` (the step reached in the study panel, per song), `chord-tutorial.ts`, `chord-listener.ts` (microphone in), `audio.ts` (synthesized notes, strums and metronome click out, on a shared lazy `AudioContext`), `harmony.ts` (notes and MIDI of a chord shape, plus the string-by-string comparison of two shapes), `notes.ts`, `chroma.ts`.

Components (`src/lib/components/`): `HeaderMenu.svelte` (the `items` array is the single source of the menu entries, rendered both inline and in the drawer), `SearchBox`, `SongSheet`, `ChordBuilder`, `ChordChange`, `ChordChecker`, `ChordTutorialCard`, `ChordHelp` (the bottom sheet of a single chord, opened from a chord tapped in a song), `StudyPanel` (the three-step study path of one song), `CapoFinder`, `FretboardMap` (`interactive` makes its cells playable), `StringTuner`, `StrumTrainer`, `QrScanner`.
