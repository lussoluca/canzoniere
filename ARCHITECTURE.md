...
A web-based song management and PDF songbook generation system for Italian scout songs, featuring a visual chord editor, category management, and event-specific songbook creation.
...
├── songbook/                # Go CLI tool: converts .txt song lists → PDF
...
- **canzonieri/**: Event songbooks. `.txt` files list song paths (relative to `canzoni/`) one per line. The Go tool (`songbook/`) reads these lists and generates PDF files with selected songs, chord annotations stripped.
- **editor/**: Interactive web app (SvelteKit) for creating, editing, and organizing songs and event songbooks.
- **songbook/**: Standalone Go program that reads a `.txt` manifest and a ChordPro song directory, then renders a text-only PDF.
...
**Parser** (`editor/src/lib/chordpro.ts`, `parse()` function, lines 50–102):
...
### PDF Generation via `songbook/` Go Tool

The `songbook` directory contains a standalone Go program that converts a `.txt` songbook manifest to a PDF.

---

## 5. The `songbook/` Go Tool — PDF Rendering
...
go build -o songbook .
cd ..
./songbook/songbook -input=canzonieri/my_event.txt -output=canzonieri/my_event.pdf
```
...
## 6. ChordPro PDF Generation (`Makefile` & `chordpro.json`)
...
Runs SvelteKit dev server on localhost:5173 (HMR enabled).
...
**Event Songbooks**:
```bash
./songbook/songbook -input=canzonieri/chiusura_2026.txt -output=canzonieri/chiusura_2026.pdf
```
...
## 8. Data Flow: From `.cho` Files to Rendered Output
...
3. **External tool generates PDF**:
   - Run `./songbook/songbook -input=canzonieri/event.txt -output=canzonieri/event.pdf`.
...
| File | Lines | Purpose |
|------|-------|---------|
| `editor/src/lib/chordpro.ts` | 1–149 | Parse/serialize ChordPro format |
| `editor/src/lib/chords.ts` | 1–107 | Chord notation & transposition |
| `editor/src/lib/categories.ts` | 1–8 | Category definitions & labels |
...
- **Rendering** (Go + Docker): Two paths: event-specific PDFs via the `songbook/` tool (text-only), or a comprehensive annotated canzoniere via the ChordPro CLI.
